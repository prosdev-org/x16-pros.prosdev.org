...
function createProgramDetailsHtml(program) {
  const tagHtml = program.category
    ? `<span class="program-modal-tag ${escapeHtml(program.category)}">${escapeHtml(program.category)}</span>`
    : "";

  return `
    <div class="program-modal-content">
      <button class="program-modal-close" aria-label="Close">&times;</button>
      <div class="program-modal-hero">
        <img class="program-modal-image" src="${escapeHtml(program.image)}" alt="${escapeHtml(program.title)} screenshot">
      </div>
      <div class="program-modal-body-inner">
        ...
        <div class="program-modal-actions">
          <a class="btn program-source-btn" href="${escapeHtml(program.sourceUrl)}" target="_blank" rel="noopener noreferrer">
...
function createMoreProgramsHtml(items) {
  ...
  return `
    ...
    <div class="program-modal-body-inner">
      ...
      <p class="program-modal-text">Additional built-in tools from x16-PRos</p>
...
function initSoftwareProgramModal(programs, morePrograms) {
  ...
  overlay.addEventListener("click", (event) => {
    if (event.target === overlay || event.target.closest(".program-modal-close")) {
